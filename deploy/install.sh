...
sudo -H -u rules bash -c "pm2 stop rules-server && cd /opt/rules-server && OPENCHS_UPLOAD_USER_USER_NAME=$1 OPENCHS_UPLOAD_USER_PASSWORD=$2 TZ=$3 NODE_ENV=production pm2 start app.js --name rules-server --update-env"
